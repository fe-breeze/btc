$(function () {
  ...
  // var coin = 'eth';
  var miner = localStorage.getItem('miner');
  // var miner = '0x444814ed8d70a6fa5581a5fa2932e7984211e63e';
  ...
  $.getJSON(url, { coin: coin, miner: miner }, function (date) {
    ...
    $.each(date.Result.Miners, function (name, fd) {
      ...
      if (now - fd.LastShare > 518) {
        ...
        $tr.append('<td><a href="http://' + coin + '.apool.cc/miner/' + miner + '.' + name + '">详情</a></td>');
    ...
    if (!offline) {
      $('.table-title li').eq(0).hide()
      $('.table-title li').eq(1).trigger('click')
    }
    ...
    $.each(date.Result.Paylog, function (i, fd) {
      ...
      $tr.append($td.clone().text(fd.Hash));
